...
def stat_card(value, label, note="", color=ACCENT, icon=""):
    ...
    st.markdown(f"""
    <div style="background:{DARK_CARD};border:1px solid {DARK_BORD};border-top:2px solid {color};
...
def section_label(text, subtitle=""):
    ...
    <div style="margin:2rem 0 1rem">
        <p style="margin:0;font-family:\'DM Serif Display\',serif;font-size:1.4rem;
                  color:{TEXT_PRI};font-weight:400">{text}</p>
        {sub}
    </div>""", unsafe_allow_html=True)
...
def callout(text, kind="info"):
    ...
    i = icons.get(kind, "◈")
    st.markdown(f"""
    <div style="border-left:3px solid {c};background:{DARK_CARD};padding:14px 18px;
        ...
        <p style="margin:6px 0 0;font-size:.88rem;color:{TEXT_PRI};line-height:1.65">{text}</p>
    </div>""", unsafe_allow_html=True)

def divider():
    st.markdown(f'<hr style="border:none;border-top:1px solid {DARK_BORD};margin:1.5rem 0">', unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────────────────
st.markdown(f"""
<div style="background:linear-gradient(135deg,{DARK_BG} 0%,#0D1829 60%,#091420 100%);
            border-bottom:1px solid {DARK_BORD};padding:3.5rem 3rem 2.5rem;position:relative;
            overflow:hidden">
    <!-- Decorative grid -->
...
with tabs[2]:
    ...
    with col2:
        ...
        for gene, note in list(FOUNDERS.items())[:8]:
            ...
            st.markdown(f"""
            <div style="border:1px solid {DARK_BORD};border-left:3px solid {ACCENT if count>10 else ACCENT2};
                ...
                <div style="display:flex;justify-content:space-between;align-items:center">
                    <span style="font-family:var(--mono);font-size:.95rem;
                                 color:{TEXT_PRI};font-weight:600">{gene}{treat_tag}</span>
                    <span style="font-family:var(--mono);font-size:.8rem;
                                 color:{ACCENT};font-weight:700">{count} cases</span>
                </div>
                <p style="margin:5px 0 0;font-size:.78rem;color:{TEXT_SEC};line-height:1.5">{note}</p>
            </div>""", unsafe_allow_html=True)
    ...
    with col2:
        ...
        for txt, col_c in [
            ...
            <div style="border-left:2px solid {col_c};padding:10px 14px;margin:10px 0;background:{DARK_CARD};border-radius:0 6px 6px 0">
                <p style="margin:0;font-size:.84rem;color:{TEXT_PRI};line-height:1.65">{txt}</p>
            </div>""", unsafe_allow_html=True)
...
with tabs[4]:
    ...
    with col2:
        ...
        for gene, tx in TREATABLE.items():
            ...
            st.markdown(f"""
            <div style="border:1px solid {DARK_BORD};border-left:2px solid {border_c};
                        background:{DARK_CARD};border-radius:0 6px 6px 0;padding:10px 12px;margin:6px 0">
                <span style="font-family:var(--mono);font-size:.9rem;color:{TEXT_PRI};font-weight:600">{gene}</span>{badge}
...
with tabs[9]:
    ...
    st.markdown(f"""
    <div style="border:1px solid {DARK_BORD};border-top:3px solid {ACCENT};
        ...
        <p style="font-size:.82rem;color:{TEXT_SEC};margin:0">
    ...
    with col1:
        ...
        for txt in [
            ...
            st.markdown(f"""
            <div style="padding:12px 0;border-bottom:1px solid {DARK_BORD}">
                <p style="margin:0;font-size:.86rem;color:{TEXT_PRI};line-height:1.7">◈ &nbsp;{txt}</p>
            </div>""", unsafe_allow_html=True)
    ...
    for name, group, desc, color in kaust_items:
        st.markdown(f"""
        <div style="border:1px solid {DARK_BORD};border-left:3px solid {color};
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-start">
                <div>
                    <p style="margin:0;font-family:'Sora',sans-serif;font-size:.95rem;
                               color:{TEXT_PRI};font-weight:600">{name}</p>
            ...
            <p style="margin:10px 0 0;font-size:.83rem;color:{TEXT_SEC};line-height:1.65">{desc}</p>
        </div>""", unsafe_allow_html=True)
    ...
    for authors, title, journal, doi in refs:
        st.markdown(f"""
        <div style="padding:8px 0;border-bottom:1px solid {DARK_BORD};display:flex;gap:12px">
...
# ─── Footer ───────────────────────────────────────────────────────────────────
st.markdown(f"""
<div style="border-top:1px solid {DARK_BORD};margin-top:3rem;padding:1.5rem 2.5rem;
